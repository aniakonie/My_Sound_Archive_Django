...
def parse_spotify_data(
    ...
    (all_playlists_tracks_library,
     playlists_to_discard) = parse_spotify_all_playlists_tracks(
         spotify_all_playlists_tracks)
    playlists_info_library = parse_playlists_info(
        spotify_playlists,
        playlists_to_discard,
        spotify_id
        )
...
def parse_track_info(track_info):
    ...
    (track_artist_main,
     main_artist_uri,
     track_artist_add1,
     track_artist_add2) = parse_track_artists_main_artist_uri(track_info)
    track_title = track_info["track"]["name"]
    (album_artist_main,
     album_artist_add1,
     album_artist_add2) = parse_album_artists(track_info)
    album_title = track_info["track"]["album"]["name"]
    album_uri = track_info["track"]["album"]["uri"]

    #TODO consider using named tuples instead of dictionaries
    #(order of the songs is not required)
    track = {
        "track_uri": track_uri,
...
def parse_track_artists_main_artist_uri(track_info):
    ...
    return (track_artist_main, main_artist_uri, track_artist_add1,
            track_artist_add2)
